...
export const getAllContactAdmin = async (currentUser,axiosJWT)=>{

    try {
   
      const res = await axiosJWT.get(`${URL}/contact/admincontact`,{
        headers:{
...
export const registerUser = async(user,dispatch,currentUser,axiosJWT,navi)=>{
    dispatch(registerStart())
    try {
     const res =   await axiosJWT.post(`${URL}/auth/adminRe`,user,{
        headers:{
            token:`Bearer ${currentUser.accessToken}`
        }
     });
        dispatch(registerSuccess());
        navi('/adminPhobendoi/allAdmin')
        if(!res.data.message){
            
               alert(`Bạn tạo thành công tại khoản: ${res.data.username}, role: ${res.data.role}`)
               return  getAllUsers(currentUser.accessToken,dispatch)
           }
...
///////Event
export const createEvent = async (dispatch,event,navigate,axiosJWT,currentUser)=>{
    dispatch( createEventStart() )
    try {
       await axiosJWT.post(`${URL}/event`,event,{
        headers:{
            token:`Bearer ${currentUser.accessToken}`
        }
       });
...
export const updateEvent= async(id,event,currentUser,axiosJWT,setisModal)=>{
    if(currentUser){

        if(currentUser.role==="1"|| currentUser.role==="admin"){
            try {
      
                const res = await axiosJWT.post(`${URL}/event/update/`+id,event,{
                    headers:{
                        token:`Bearer ${currentUser.accessToken}`
                    }
                });
