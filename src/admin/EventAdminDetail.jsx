...
import ReadQuillEditor from "../component/Ultity/ReadOnlyEditor";
...
if(isLoading){
  ...
  return ( 
    ...
    <div className=" h-[auto] flex pt-[15vh]    flex-col items-center w-[100%] bg-primary ">
      ...
      <button onClick={()=>setOnlinePaymentlist(false)} className=" rounded-xl bg-primaryYellow w-[10vw] border-2 h-[3vw] ">Back to even info</button>
      ...
      {isModal?<UpdateEvent setisModal={setisModal} update={onlinePaymentlist} ></UpdateEvent>:"" }
      {onlinePaymentlist?
                                    ...
                                    item3.onlinePayment.map((item4,i)=>{
                                      return(
                                        <tr key={i} className="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                                          ...
                                          <th scope="row" className={`${item4.isPaid?"bg-green-400":" bg-red-400"} shrink-0 w-[10%] px-6 py-4 font-medium text-white dark:text-white whitespace-nowrap`}>
                                          {item4.name}
                                            </th>
                                          <td  className="px-6 py-3  shrink-0 w-[10%]">  {item4.eventCode} </td >
                                          <td  className="px-6 py-3  shrink-0 w-[50%]">  
                                          Email: {item4.buyerInfo?.email?item4.buyerInfo?.email:"chưa có"} 
                                          Họ và tên:  {item4.buyerInfo?.username||"chưa có"} 
                                          Địa chỉ: {item4.buyerInfo?.address||"chưa có"}
                                          SĐT: {item4.buyerInfo?.phone||"chưa có"}
                                          </td >
                                      
                                          <td  className=" px-6 py-3 shrink-0 w-[10%]">  {item4.slot} </td >
                                          <td  className=" px-6 py-3 shrink-0 w-[10%]">  {`${new Intl.NumberFormat('it-IT', { style: 'currency', currency: 'VND' }).format(item4.slot*item4.costPerTicket)}`} </td >
      ...
      <div className=" flex text-aCaption uppercase h-[35%]   ">
       ...
       <div className=" mt-[12vh] h-[auto] py-[5%] flex">
        ...
        <ReadQuillEditor className="text-aPara font-p" value={event.discription}></ReadQuillEditor>
