...
function UpdateEvent({setisModal}) {
  ...
  const handleSubmit = async()=>{
      const res = await updateEvent(eventID,Event,currentUser,axiosJWT,setisModal)
  ...
  return ( 
    ...
    <div> <button onClick={()=>setArtist(artist=>[...artist,`Hãy điền tên nghệ sĩ mới thêm`])} className="rounded-xl w-[10vw] text-white text-center bg-pink-400 h-[3vw]"> Thêm nghệ sĩ</button> </div>
      ...
      <QuillEditor handleDelta={handleDelta} defaultValue={nodes} value={nodes}  ></QuillEditor>
 Review view:
