...
try {
  const res = await axiosJWT.post('https://api.phobendoi.art/api/discount/create',data,{
    headers:{
      token:`Bearer ${currentUser.accessToken}`
  }
  })
  alert("Đã tạo thành công vé",res.data.name)
  const res2 =  await axios.get('https://api.phobendoi.art/api/discount')
  return setlistGiftCode(res2.data)
} catch (error) {
 ...
 const success = ()=>{
  ...
  const getOne = async (id)=>{
    ...
    const res =  await axios.get('https://api.phobendoi.art/api/discount/'+id)
    setIUpdate(res.data)
    return 
  }
  ...
  const columns = [
    ...
    },
    {
      ...
      render:(val,row,i)=>{
        ...
        const check = compareAsc(time,now)
        
        if(check===-1){
          return <span className=' text-white px-1 py-2 bg-red-500'>{prse}</span>
        }
        if(check===0){
          return <span className=' text-white px-1 py-2 bg-yellow-400'>{prse}</span>
        }
        if(check===1){
          return <span className='text-white px-1 py-2 bg-green-400'>{prse}</span>
        }
  ...
  useEffect(()=>{
    (async () => {
      ...
      const res =  await axios.get('https://api.phobendoi.art/api/discount')
  ...
  return (  
    ...
    <ButtonBack></ButtonBack>
      ...
      {isUpdate? 
      <div>
      <BackState action={()=>setIUpdate(false)}  ></BackState>
      <UpdateGiftcode success={success} data ={isUpdate} ></UpdateGiftcode>
    </div>
     :""}
