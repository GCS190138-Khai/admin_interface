...
import { createAxios,  modules, toHtml, toText } from "../../createInstance";
import { useEffect } from "react";
import EditorLex from "../test/Editor_Lexical";
import QuillEditor from "../Ultity/Editor";
...
function InputEvent() {
  ...
  const [HeroPic, setHeroPic] = useState('sample')
  ...
  const Event={
    ...
    eventCode:EventCode, 
    heroPic:`https://api.phobendoi.art/uploads/events/${HeroPic[0].name.trim()}`,
    artist:artist,
    duration:duration,
    startMin:startMin,
    startHour:startHour,
    ...
    try {
      const formData = new FormData()
       ...
       formData.append('image',HeroPic[0])
      
       await axiosJWT.post('https://api.phobendoi.art/api/event/upload',formData,{
      ...
      const res =  await createEvent(dispatch,Event,navigation,axiosJWT,currentUser)
        alert("thanh cong")
    } catch (error) {
      alert(error)
...
const handleOnchangeDeleteTicket =(index,arr)=>{
  ...
  return ( 
    ...
    <div className=" h-[auto] flex pt-[20vh]  px-[2%]  flex-col items-center w-[100%] bg-primary ">
      ...
      {!onlinePaymentTab?<div className="  w-[100%] h-[auto]  flex flex-col " >
      ...
      <div className=" border-b border-primaryBlack"></div>
      <label htmlFor="heroPic"> heroPicture </label><input required type="file"  onChange={(e)=>
      {
        setHeroPic(e.target.files)
      
    }
      } id="heroPic" placeholder="hãy dán link ảnh đã up vào đây. ví dụ: http://"  className="block p-4 pl-10 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" />
      <div className=" border-b border-primaryBlack"></div>
 ...
 {/* <ReactQuill modules = {modules } className=" break-words w-[49.563rem]" theme="snow" value={nodes} onChange={handleDelta} /> */}
 <QuillEditor handleDelta={handleDelta} value={nodes}  ></QuillEditor>
 Review view:
 <ReactQuill  style={{ height:"fit-content"}} className="break-words w-[49.563rem]" theme="bubble" value={delta} readOnly={true}/>
    
   
   
      </div>
      <div className=" border-b w-[78%]  border-primaryBlack"></div>
     <label  htmlFor="EventStatus">EventStatus</label> 
     <select defaultValue={0} onChange={(e)=>setEventStatus(e.target.value)}  className="w-[20vw] bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" name="EventStatus" id="EventStatus">
...
<div className=" border-b w-[78%]  border-primaryBlack"></div>
  <label htmlFor="TicketLimitationAtOnce"> Có giới hạn mua vé là bao nhiêu ? </label><input id='TicketLimitationAtOnce' onChange={e=>setTicketLimitationAtOnce(e.target.value)} value={TicketLimitationAtOnce} type="number" min="0" max="20" pattern="[+]?[0-9]" name="TicketLimitationAtOnce" className="block p-4 pl-10 w-[10%] text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" />
    ...
    <div className=" border-b w-full  border-primaryBlack"></div>
    ...
    <input type="checkbox" defaultChecked={isHidden} value={isHidden} onChange={()=>{setisHidden(!isHidden)
