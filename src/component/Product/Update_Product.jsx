...
const schema = yup.object().shape({
  pics: yup.mixed().test("1test","Chỉ được add 5 hình",(val)=>{
...
function UpdateProduct({data,refetch}) {
  ...
  const [listOfGen, setlistOfGen] = useState(data.genres)
  const [picList, setpicList] = useState([])

  const [imagesList, setImagesList] = useState([]);
  let [nodes, setNodes] = useState(data.discription);
 ...
 let newOption = []
 _.forEach(data.option,(item,i)=>{
  if(!item.thumnailPics) {
    return newOption.push({...item,thumnailPics:rawData.option[i].thumnailPics})
}else if(item.thumnailPics.length===0){
  return newOption.push({...item,thumnailPics:rawData.option[i].thumnailPics})
  }else{
  ...
  isHidden:data.isHidden
   ...
   if(newVal.length===0){
    ...
    const res = await axiosJWT.put('https://api.phobendoi.art/api/products/update/'+idData,newProduct,{
      headers:{
   ...
   updateProduct.mutate({product,rawData})
      alert("Bạn update thành công sản phẩm:",res.data.name)
    }
    ...
    return (  
      <div className="dv"> 
        <form onSubmit={handleSubmit((e)=>{
          ...
          handleAddCity(e)
        })} className=' flex flex-col gap-[15px] h-fit w-full  '>
                 ...
                 {option.map((item, index) => {
                   ...
                   return (
                      <li className=' border-2 rounded-sm  h-[60vh] gap-[2vh] flex flex-col justify-between  ' key={item.id}>
                       ...
                       <input  type={"file"} className=' rounded-xl pb-[0.875vh] focus:border-primaryBlack  focus:ring-0 w-full  inp' {...register(`option.${index}.thumnailPics`)} />
                  ...
                  <button  type='submit'   className=' w-fit flex cursor-pointer'> <img className=" ml-[-0.5rem] object-contain h-[2rem] w-[2rem] " src={ "https://live.staticflickr.com/65535/52252454497_7a572f16d1_o.png" } alt="" /> <u className="mt-[0.8rem] font-bold text-[0.875rem]">Cập nhật sản phẩm</u> </button>
              </form>
      </div>
